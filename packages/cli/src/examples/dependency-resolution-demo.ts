...
async function demonstrateDependencyResolution() {
  ...
  for (const [from, to] of checks) {
    const wouldCircular = wouldCreateCircularDependency(from, to, registry);
    logger.info(`${from} -> ${to}: ${wouldCircular ? '❌ Would create cycle' : '✅ Safe'}`);
  }
  logger.info('');
  ...
  logger.info(`Average dependencies per component: ${(totalDeps / registry.components.size).toFixed(2)}`);
