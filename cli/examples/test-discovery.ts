...
import { discoverComponents, getDiscoveryStats, getComponentsByCategory, searchComponents } from '../lib/components.js';
...
async function testDiscovery(): Promise<void> {
  ...
  try {
    ...
    const validationComponents = searchComponents('validation', registry, { includeDescription: true });
    ...
    console.log('\n📄 Sample Components:');
    Array.from(registry.components.values()).slice(0, 5).forEach(comp => {
      console.log(`   ${comp.metadata.name} (${comp.type})`);
